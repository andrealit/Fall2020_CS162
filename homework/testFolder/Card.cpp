 ...
 * Post-Conditions: -
 */
void Card::set_location(int loc)
{
    this->location = loc;
}
...
//  * Description: displays where the card is
//  * Parameters: -
//  * Pre-Conditions: -
//  * Post-Conditions: -
//  */
// void Card::show_location() {
...
bool Card::playable(Card faceup)
{
    ...
    if (this->get_rank() == faceup.get_rank() || this->get_suit() == faceup.get_suit())
    {
        cout << "The card is playable!" << endl;
        canPlay = true;
    }

    if (faceup.get_rank() == 9) {
        cout << "CRAZY 8s!" << endl;
        canPlay = true;
    }
...
/**
 * Function: play_card
 * Description: actually play the card
 * Parameters: -
 * Pre-Conditions: The card we selected is not yet played
 * Post-Conditions: The card is played
 */
void Card::play_card()
{
    // logic: once a card is played, the location has changed
}
